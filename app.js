...
async function renderLesson(){
  ...
  $('lessonTitle').innerHTML=`<h2>${lessonLabel(state.lesson)}</h2>`;
...
async function addPages(files){
  ...
  for(const f of files){
    await put('pages',{id:uid(),lesson:state.lesson,order:order++,pageNo:guessPageNo(f.name,order),
      // 最初に追加したページ=発音ページ、それ以降=解説ページ
      visible:(order===1),blob:f,name:f.name});
  }
  await rerender();
}
...
async function addAudios(files){
  ...
  let order=existing.length;
  for(const f of files){
    await put('audios',{id:uid(),lesson:state.lesson,order:order++,name:f.name,blob:f});
  }
  await rerender();
}
